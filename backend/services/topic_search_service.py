...
def _get_openrouter_model() -> str:
    """Retrieve the configured OpenRouter model from settings or environment."""
    model = (getattr(settings, "openrouter_model", "") or os.getenv("OPENROUTER_MODEL", "")).strip().strip('"').strip("'")
    return model or "openrouter/auto"


def _resolve_openrouter_model(model_choice: str = "nemotron") -> list[str]:
    """Return ordered candidate OpenRouter models, preferring the configured router model."""
    configured_model = _get_openrouter_model()
    requested_model = (model_choice or "nemotron").strip().lower()

    candidate_models = []
    if requested_model in MODEL_MAP:
        candidate_models.append(MODEL_MAP[requested_model])
    elif requested_model and requested_model not in ("default", "custom"):
...
"""
    ...
    if openrouter_key:
        candidate_models = _resolve_openrouter_model(model_choice)
        ...
        for attempt_index, model_id in enumerate(candidate_models):
            ...
            try:
                async with httpx.AsyncClient(timeout=45.0) as client:
                    ...
                    print(f"[TopicSearch] OpenRouter returned HTTP {res.status_code} for model={model_id}.", flush=True)
                    if res.status_code in (400, 404) and model_id != "openrouter/auto":
                        print("[TopicSearch] Retrying with OpenRouter auto-router fallback...", flush=True)
                        continue
                    break
            except Exception as e:
                print(f"[TopicSearch] OpenRouter request failed for model={model_id}: {e}", flush=True)
